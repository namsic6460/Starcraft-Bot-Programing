...
public class WorkerManager {
	...
	public static boolean nonGasTiming = false;
...
//			if(worker.isIdle())
//				System.out.println(workerData.getWorkerJob(worker) + " " + worker.getID());
		}
	}
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits())
		{
			...
			if (unit.getType().isRefinery() && unit.isCompleted() )
			{
				...
				if(!nonGasTiming) {
					...
					if(workerData.getWorkerJob(unit) == WorkerData.WorkerJob.Gas) {
	...
	public void setConstructionWorker(Unit worker, UnitType buildingType)
	{
		...
		if(workerData.getWorkerJob(worker) != WorkerData.WorkerJob.Gas && workerData.getWorkerJob(worker) != WorkerData.WorkerJob.Build)
