...
public class WorkerManager {
	...
	public void handleGasWorkers()
	{
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits())
		{
			...
			if (unit.getType().isRefinery() && unit.isCompleted() && unit != StrategyManager.Instance().enemyGeyser)
			{
	...
	public void handleIdleWorkers() 
	{
		...
		for (Unit worker : workerData.getWorkers())
		{
			...
			if(StrategyManager.Instance().waitBuild && worker == StrategyManager.Instance().gasWorker)
				continue;
	...
	public void handleMoveWorkers()
	{
		...
		for (Unit worker : workerData.getWorkers())
		{
			if (worker == null || StrategyManager.Instance().gasWorker != null) continue;
