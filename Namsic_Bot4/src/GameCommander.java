...
public class GameCommander {
	...
	public static HashMap<UnitType, ArrayList<Unit>> units = new HashMap<>();
	...
	public void onUnitCreate(Unit unit) {		
		...
		if (unit.getPlayer() == MyBotModule.Broodwar.self()) {
			...
			if (!units.containsKey(unitType))
				units.put(unitType, new ArrayList<Unit>());
	...
	public void onUnitDestroy(Unit unit) {
		...
		if (unit.getPlayer() == MyBotModule.Broodwar.self()) {
			UnitType unitType = unit.getType();
			units.get(unitType).remove(unit);
		}
	}
	...
	public void onUnitRenegade(Unit unit) {
		...
		UnitType unitType = unit.getType();
		if(unit.getPlayer() == MyBotModule.Broodwar.self()) {
			if(!units.get(unitType).contains(unit))
				units.get(unitType).add(unit);
		}
		
		else if(units.get(unitType).contains(unit))
			units.get(unitType).remove(unit);
	}
