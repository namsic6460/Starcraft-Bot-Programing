...
public class ScoutManager {
	...
	private List<BaseLocation> bases = null;
	private List<Position> explored = new ArrayList<>();
	private List<Position> skip = new ArrayList<>();
	private int preBaseNum = -1;
	private int baseNum = 0;
	private int checkCanGo = 0;
	...
	public void assignScoutIfNeeded()
	{
		...
		if (enemyBaseLocation == null || (enemyBaseLocation != null && MyBotModule.Broodwar.getFrameCount() > 16000))
		{
			if (currentScoutUnit == null || currentScoutUnit.exists() == false || currentScoutUnit.getHitPoints() <= 0)
			{
				...
				if (StrategyManager.Instance().needsScout || firstBuilding != null)
				{
					...
					if (unit != null)
					{
						...
						StrategyManager.Instance().gasWorker = unit;
						WorkerManager.Instance().setScoutWorker(currentScoutUnit);
						
						StrategyManager.Instance().needsScout = false;
		...
		else 
		{
			...
			if (currentScoutUnit != null) {
				...
				if (MyBotModule.Broodwar.isExplored(currentScoutTargetBaseLocation.getTilePosition()) == false) {
					...
					if (MyBotModule.Broodwar.getFrameCount() < 16000) {
						WorkerManager.Instance().setIdleWorker(currentScoutUnit);
						currentScoutStatus = ScoutStatus.NoScout.ordinal();
						...
						if(bases == null) {
							bases = BWTA.getBaseLocations();
						
							for(BaseLocation base : bases) {								
								if(checkPos(base, InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().selfPlayer)) ||
										checkPos(base, InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().enemyPlayer)) ||
										checkPos(base, InformationManager.Instance().getFirstExpansionLocation(InformationManager.Instance().selfPlayer)) ||
										checkPos(base, InformationManager.Instance().getFirstExpansionLocation(InformationManager.Instance().enemyPlayer)))
									skip.add(base.getPoint());
							}
						}
						...
						if (preBaseNum != baseNum) {
							if (checkPos(skip, currentScoutTargetPosition))
								baseNum++;

							if (baseNum >= bases.size()) {
								baseNum = 0;
								explored.clear();
							}
						...
						currentScoutStatus = ScoutStatus.MovingToAnotherBaseLocation.ordinal();
						currentScoutTargetPosition = bases.get(baseNum).getPoint();
						currentScoutTargetBaseLocation = bases.get(baseNum);
						
						if(!explored.contains(currentScoutTargetPosition)) {
							double distance = BWTA.getGroundDistance(currentScoutUnit.getTilePosition(), currentScoutTargetBaseLocation.getTilePosition());
							commandUtil.move(currentScoutUnit, currentScoutTargetPosition);

							if(distance < 250 && distance > 0) {						
								explored.add(currentScoutTargetPosition);
								baseNum++;
							}
								...
								if(checkCanGo > 10) {
									explored.add(currentScoutTargetPosition);
									skip.add(currentScoutTargetPosition);
									baseNum++;
								}
							}
						}
	...
	private boolean checkPos(BaseLocation base1, BaseLocation base2) {
		if(base1.getX() == base2.getX() && base1.getY() == base2.getY())
			return true;
		return false;
	}
	
	private boolean checkPos(List<Position> list, Position pos) {
		for(Position p : list) {
